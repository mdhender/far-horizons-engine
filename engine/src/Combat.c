...
/* Add coordinates to list if not already there. */
                ...
                if (!found) { /* This is a new battle location. */
                    ...
                    bat->haven_x[0] = 127; /* 127 means not yet specified. */
                    ...
                    num_battles++;
                } else { /* Add another species to existing battle location. */
...
/* Check if this is an order to attack all declared enemies. */
                if (get_value() && value == 0) {
                    for (i = 0; i < galaxy.num_species; i++) {
                        ...
                        enemy_word_number = i / 32; /* always the zero-based index */
