...
extern int log_summary, ignore_field_distorters, log_to_file;
...
do_round(char option, int round_number, struct battle_data *bat, struct action_data *act) {
    ...
    while (total_shots > 0) {
         ...
         *  The attacker will place more emphasis on the use of these devices
         *  if he emphasizes gravitics technology over military technology. */
        fj_chance = 50 * attacker_gv / attackerGvMl;
        if (rnd(100) < fj_chance && act->unit_type[attacker_index] == SHIP && act->unit_type[defender_index] == SHIP) {
