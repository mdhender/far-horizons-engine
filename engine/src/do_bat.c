...
int attacking_ML, defending_ML, deep_space_defense, ambush_took_place;
char field_distorted[MAX_SPECIES], append_log[MAX_SPECIES], make_enemy[MAX_SPECIES][MAX_SPECIES];


extern int attacker_here, defender_here, logging_disabled, strike_phase, prompt_gm;
...
do_battle(struct battle_data *bat) {
    int i, j, k, species_index, species_number, num_sp, save, max_rounds;
    int round_number, battle_here, fight_here, unit_index, option_index;
    int current_species, temp_status, temp_pn, num_namplas, array_index;
    int bit_number, first_action, traitor_number, betrayed_number, betrayal;
    int need_comma, TRUE_value, do_withdraw_check_first;
    ...
    for (species_index = 0; species_index < num_sp; ++species_index) {
        ...
        for (i = 0; i < c_species[species_index]->num_ships; i++) {
            ...
            if (sh->age < 50) {
                ...
                continue;
            } else if (sh->status == UNDER_CONSTRUCTION) { continue; }
            delete_ship(sh);
        }
    }
}
...
void auto_enemy(int traitor_species_number, int betrayed_species_number) {
    int traitor_array_index, betrayed_array_index, bit_number, species_index;
    ...
    for (species_index = 0; species_index < galaxy.num_species; species_index++) {
        if ((spec_data[species_index].ally[traitor_array_index] & traitor_bit_mask) == 0) {
            continue;
        } else if ((spec_data[species_index].ally[betrayed_array_index] & betrayed_bit_mask) == 0) {
            continue;
        } else if ((spec_data[species_index].contact[traitor_array_index] & traitor_bit_mask) == 0) {
            continue;
        } else if ((spec_data[species_index].contact[betrayed_array_index] & betrayed_bit_mask) == 0) {
            continue;
        }
