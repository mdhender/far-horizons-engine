...
jump_end:
	...
	for (nampla_index = species->num_namplas - 1; nampla_index >= 0; nampla_index--) {
	    ...
	    if (nampla->status & MINING_COLONY) {
			...
			fprintf (report_file, "    ;  Avail pop = %ld, shipyards = %d, to spend = %ld", nampla->pop_units, nampla->shipyards, nampla->use_on_ambush);
	    ...
	    if (nampla->IUs_needed) {fprintf (report_file, "\tBuild\t%d IU\n", nampla->IUs_needed);}
	    if (nampla->AUs_needed) {fprintf (report_file, "\tBuild\t%d AU\n", nampla->AUs_needed);}
	    if (nampla->IUs_needed  ||  nampla->AUs_needed) {fprintf (report_file, "\n");}
	    ...
	    n = nampla->special / 5;
	    if (n > 0) {fprintf (report_file, "\tRecycle\t%d RM\n\n", 5*n);}
	    ...
	    n += nn;
	    if ((nampla->status & COLONY)  &&  n < 2000L &&  nampla->pop_units > 0) {
	    ...
	    if (n >= 2000L  ||  (nampla->status & HOME_PLANET)) {
			...
			for (i = 1; i < species->num_namplas; i++) {
				...
				if (nn > temp_nampla->item_quantity[CU]) {nn = temp_nampla->item_quantity[CU];}
				n += nn;
				if (n >= 2000L) {continue;}
				nn = 2000L - n;
...
do_planet_report (struct nampla_data *nampla, struct ship_data *s_base, struct species_data *species) {
    ...
    raw_material_units = (10L * (long) species->tech_level[MI] * (long) nampla->mi_base) / (long) planet->mining_difficulty;
